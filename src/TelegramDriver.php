...
class TelegramDriver extends HttpDriver
{
    ...
    public function getUser(IncomingMessage $matchingMessage)
    {
        ...
        if ($response->getStatusCode() !== 200) {
            throw new TelegramException('Error retrieving user info: '.json_encode($parameters,JSON_UNESCAPED_UNICODE).'|'.$response->getContent());
        }
	...
	if($messageText == '%%%_FILE_%%%' || $messageText == '%%%_IMAGE_%%%' || $messageText == '%%%_PHOTO_%%%' || $messageText == '%%%_AUDIO_%%%' || $messageText == '%%%_VIDEO_%%%') {
            die();
	}
